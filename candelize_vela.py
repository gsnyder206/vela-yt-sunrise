...
def process_snapshot(subdirpath='.',mockimage_parameters=None,clobber=False, max=None, galaxy=None,seg_filter_label='NC-F200W',magsb_limits=[23.0,25.0,27.0,29.0],camindices='All',do_idl=False,analyze=True,use_nonscatter=True,Np=2,smc=False):
    ...
    filter_labels = ['WFC3-F336W',
                     ...
                     'WFI-R062','WFI-Z087','WFI-Y106','WFI-W146','WFI-F184',
    ...
    photfnu_Jy = [4.93e-7,
                  ...
                  1.17e-7,1.52e-7,
                  3.17e-8,3.17e-8,3.17e-8,3.17e-8,3.17e-8,  #fix WFI photfnu to NIRCAM F115W -- similar instrument, middle of wl range, only matters for morph statistics
    ...
    psf_names = ['gen6runs/F336W_rebin.fits',
                 'gen6runs/F435W_rebin.fits','gen6runs/F606W_rebin.fits','gen6runs/F814W_rebin.fits',
                 'gen6runs/F125W_rebin.fits','gen6runs/F160W_rebin.fits',
                 'gen6runs/WFI_R062_SCA01_center_psf_phaseb.fits',
                 'gen6runs/WFI_Z087_SCA01_center_psf_phaseb.fits',
                 'gen6runs/WFI_Y106_SCA01_center_psf_phaseb.fits',
                 #'gen6runs/WFI_J129_SCA01_center_psf_phaseb.fits',
                 'gen6runs/WFI_W149_SCA01_center_psf_phaseb.fits', #note filter name inconsistency with older version of WEBBPSF
                 #'gen6runs/WFI_H158_SCA01_center_psf_phaseb.fits',
                 'gen6runs/WFI_F184_SCA01_center_psf_phaseb.fits',
                 'gen6runs/oversamp_WebbPSF_F115W_trunc.fits','gen6runs/oversamp_WebbPSF_F150W_trunc.fits','gen6runs/oversamp_WebbPSF_F200W_trunc.fits',
                 'gen6runs/oversamp_WebbPSF_F277W_trunc.fits','gen6runs/oversamp_WebbPSF_F356W_trunc.fits','gen6runs/oversamp_WebbPSF_F444W_trunc.fits',
                 'gen6runs/oversamp_WebbPSF_F770W_trunc.fits','gen6runs/oversamp_WebbPSF_F1500W_trunc.fits']
    ...
    psf_fwhm = [0.08,
                0.10,0.11,0.13,
                0.17,0.20,
                0.085*0.62,0.085*0.87,0.085*1.06,0.085*1.46,0.085*1.84,
                0.11,0.11,0.12,
                0.15,0.18,0.25,
...
if __name__=="__main__":
    ...
    try:
        assert(sys.argv[2]=='limit')
        ci=[0,1,2]
    except:
        ci='All'

    try:
        res1 = process_snapshot(subdirpath='.',camindices=ci, clobber=False,seg_filter_label='NC-F200W',magsb_limits=[25.0,28.0],do_idl=False,analyze=False,use_nonscatter=False,smc=False,Np=4)
    except:
        print('failure in candelization of broadbandz.fits, scatter')

    try:
        res2 = process_snapshot(subdirpath='.',camindices=ci, clobber=False,seg_filter_label='NC-F200W',magsb_limits=[25.0,28.0],do_idl=False,analyze=False,use_nonscatter=True,smc=False,Np=4)
    except:
        print('failure in candelization of broadbandz.fits, nonscatter')

    try:
        res3 = process_snapshot(subdirpath='.',camindices=ci, clobber=False,seg_filter_label='NC-F200W',magsb_limits=[25.0,28.0],do_idl=False,analyze=False,use_nonscatter=False,smc=True,Np=4)
    except:
